...
float pitch_gain = 0; // pitch gain (10)
float derivative_gain = 0; // pitch derivative gain (3)
float integral_pitch = 0; // integral pitch
float integral_gain = 0; // pitch integral gain (.3)
float integral_saturate = 300; // max and min integral value
...
void set_motor_values()
{
  ...
  /* roll */
  float roll_measured = roll_angle;
  ...
  // X-frame mixing: motors 0,1 = left side, motors 2,3 = right side
  // motors 0,2 = front, motors 1,3 = back
  // if roll response is backwards, flip sign of roll_pid
  motor_commands[0] = (int)(thrust + pitch_pid - roll_pid); // front-left
  motor_commands[1] = (int)(thrust - pitch_pid - roll_pid); // back-left
  motor_commands[2] = (int)(thrust + pitch_pid + roll_pid); // front-right
  motor_commands[3] = (int)(thrust - pitch_pid + roll_pid); // back-right

  print_counter++;
  if(true)
    printf("%.4f %d %d %d %d %.4f %.4f\n", program_time,
           motor_commands[0], motor_commands[1], motor_commands[2], motor_commands[3],
           roll_angle * 100, roll_desired * 100);
}
