...
float thrust_amplitude=450; // joystick thrust read
...
float integral_pitch = 0; // integral pitch
float integral_gain = 0; // pitch I gain (zeroed for yaw test)
float integral_saturate = 300; // max and min integral value

float roll_amplitude = 10; // joystick roll read (degrees max)
float roll_gain = 0; // roll P gain (zeroed for yaw test)
float roll_derivative_gain = 0; // roll D gain (zeroed for yaw test)
float integral_roll = 0; // roll integral accumulator
...
void set_motor_values()
{
  ...
  if(print_counter % 20 == 0){}
    printf("%.4f %d %d %d %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n", program_time,
           ...
           pitch_error, roll_error, roll_angle, roll_desired, thrust, yaw_rate*10, yaw_desired*10);
}
